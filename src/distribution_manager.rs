...
use amethyst::ecs::{prelude::ReadStorage, Entity};
...
impl DistributionManager {
    ...
    pub fn request_new_task<'s>(
        &mut self,
        task_type: GUnitType,
        unit: GUnitAttributes,
        storage: &ReadStorage<'s, PlatformAttributes>,
    ) -> Option<Task> {
            ...
            GUnitType::Idle => {
                if let Some(platform) = unit.get_current_platform() {
                    let platform = storage
                        .get(*platform)
                        .expect("Failed to get Platform Attributes");
                    let connected = platform.get_connected();

                    if connected.len() > 0 {
                        return Some(Task {
                            end_platform: connected.choose(&mut rand::thread_rng()).copied(),
                            ..task
                        });
                    }
                }
